...
int get_player_data() { 
   ...
   if(fd == -1) // ファイルをオープンできない。　おそらくファイルが存在していない。
      return -1; 
   read_bytes = fread(fd, &entry, sizeof(struct user));    // 最初のかたまりを読み込む。
   while(entry.uid != uid && read_bytes > 0) { // 正しいuidが見つかるまで繰り返す。
      read_bytes = fread(fd, &entry, sizeof(struct user)); // 読み込みを続ける。
   }
   pclose(fd); // ファイルをクローズする。
...
void register_new_player()  { 
   ...
   fwrite(fd, &player, sizeof(struct user));
   pclose(fd);
...
void update_player_data() {
   ...
   fread(fd, &read_uid, 4);          // 最初の構造体からuidを読み込む。
   while(read_uid != player.uid) {  // 正しいuidが見つかるまで繰り返す。
      for(i=0; i < sizeof(struct user) - 4; i++)  // この構造体の残りを
         fread(fd, &burned_byte, 1);               // 読み込む。
      fread(fd, &read_uid, 4);       // 次の構造体からuidを読み込む。
   }
   fwrite(fd, &(player.credits), 4);   // クレジットを更新する。
   fwrite(fd, &(player.highscore), 4); // ハイスコアを更新する。
   fwrite(fd, &(player.name), 100);    // ユーザ名を更新する。
   pclose(fd);
}
...
void show_highscore() {
   ...
   while(fread(fd, &entry, sizeof(struct user)) > 0) { // ファイルの終端まで繰り返す。
